...
class tc_vfmv_v_f_seq(vfadd_base_seq):
...
class TEST_VFMV_V_F(BaseTestCase):
    def create_sequence(self):
        self.seq = tc_vfmv_v_f_seq(self.env.vfadd_master_agent)

@toffee_test.testcase
async def test_vfmv_v_f(vfadd_setup):
    test = TEST_VFMV_V_F(vfadd_setup)
    await test.run()
